    ...
    def getData(self, dxl_id, address, data_length):
        ...
        if data_length == 1:
            ...
            return DXL_MAKEDWORD(DXL_MAKEWORD(self.data_dict[dxl_id][address - self.start_address + 0],
                ...
                ### Edit ### Read neighbor data like present velocity and present position
        elif data_length == 8:
            print("works!")
            data1 = DXL_MAKEDWORD(DXL_MAKEWORD(self.data_dict[dxl_id][address - self.start_address + 0],
                                              self.data_dict[dxl_id][address - self.start_address + 1]),
                                DXL_MAKEWORD(self.data_dict[dxl_id][address - self.start_address + 2],
                                              self.data_dict[dxl_id][address - self.start_address + 3]))
            data2 = DXL_MAKEDWORD(DXL_MAKEWORD(self.data_dict[dxl_id][address - self.start_address + 4],
                                              self.data_dict[dxl_id][address - self.start_address + 5]),
                                DXL_MAKEWORD(self.data_dict[dxl_id][address - self.start_address + 6],
                                              self.data_dict[dxl_id][address - self.start_address + 7]))
            return data1, data2
