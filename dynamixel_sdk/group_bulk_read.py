...
class GroupBulkRead:
    ...
    def getData(self, dxl_id, address, data_length):
        ...
        if data_length == 1:
            ...
            #print("bulk read works!")
            ## In case of BulkRead Velocity/Position
            data1 = DXL_MAKEDWORD(DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 0],
                                              self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 1]),
                                 DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 2],
                                              self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 3]))
            data2 = DXL_MAKEDWORD(DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 4],
                                              self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 5]),
                                 DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 6],
                                              self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 7]))
            return data1, data2

        elif data_length == 10:
            #print("bulk read works!")

            ## In case of BulkRead Torque/Velocity/Position
            data1 = DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr],
                                self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 1])
            data2 = DXL_MAKEDWORD(DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 2],
                                              self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 3]),
                                 DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 4],
                                              self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 5]))
            data3 = DXL_MAKEDWORD(DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 6],
                                              self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 7]),
                                 DXL_MAKEWORD(self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 8],
                                              self.data_dict[dxl_id][PARAM_NUM_DATA][address - start_addr + 9]))
